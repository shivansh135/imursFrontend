...
utils.detectHash = function () {
  ...
  Array.prototype.forEach.call(utils.getSharePrefixes(), function (prefix) {
    if (defaults_DEARVIEWER.preParseHash && defaults_DEARVIEWER.preParseHash.indexOf(prefix) >= 0 && defaults_DEARVIEWER.hashFocusBookFound === false) {
      var id = defaults_DEARVIEWER.preParseHash.split(prefix)[1].split('/')[0];
      var page = defaults_DEARVIEWER.preParseHash.split(prefix)[1].split('/')[1];
      if (page != null) {
        page = page.split('/')[0];
      }
      var book;
      ...
      //then id pattern
      if (book.length === 0) book = utils_jQuery('#' + id);
      //then _slug pattern
    ...
    if (isParsed !== "true") {
      ...
      if (lightboxType === void 0) {
        ...
        if (lightboxType === "thumb") {
          var thumbLayout = app.data("thumb-layout") || defaults_DEARVIEWER.defaults.thumbLayout;
          ...
          if (defaults_DEARVIEWER.defaults.displayLightboxPlayIcon === true) app.addClass("df-icon-play");
          if (thumbLayout !== "custom") {
            app.html("");
            var title = utils_jQuery("<span class='df-book-title'>").html(innerText);
            var wrapper = utils_jQuery("<div class='df-book-wrapper'>").appendTo(app);
            var cover = utils_jQuery("<div class='df-book-cover'>").append(title).appendTo(wrapper);
            if (thumbURL == null || thumbURL.toString().trim() == '') {
              app.addClass("df-thumb-not-found");
              thumbURL = defaults_DEARVIEWER.defaults.imagePlaceholder || placeholderImage;
            }
            var image = utils_jQuery('<img alt="' + innerText + '"/>');
...
var BaseViewer = /*#__PURE__*/function () {
  ...
  _createClass(BaseViewer, [{
    ...
    value: function getRenderContext(pdfPage, param) {
      ...
      dimen.height = base_viewer_utils.limitAt(dimen.height, 1, app.dimensions.maxTextureHeight);
    ...
    value: function mouseWheel(event) {
      ...
      if (app.options.mouseScrollAction === defaults_DEARVIEWER.MOUSE_SCROLL_ACTIONS.NAV && isValidZoom === false) {
        if (delta > 0) {
          app.next();
        } else if (delta < 0) {
          app.prev();
        }
      } else if (app.options.mouseScrollAction === defaults_DEARVIEWER.MOUSE_SCROLL_ACTIONS.ZOOM || isValidZoom === true) {
        if (delta > 0 || delta < 0) {
          event.preventDefault();
...
var Reader = /*#__PURE__*/function (_BaseViewer) {
  ...
  reader_createClass(Reader, [{
    ...
    value: function _getInnerHeight() {
      ...
      if (this.app.pageScaling === defaults_DEARVIEWER.PAGE_SCALE.ACTUAL) maxWidth = this.app.provider.coverPage.viewPort.width * 1;
      ...
      this._containCover = reader_utils.contain(viewPort.width * 100, viewPort.height * 100, maxWidth, maxHeight);
    ...
    value: function resize() {
      ...
      // if (app.provider.coverPage) {//todo_old this shouldn't be called before coverpage
...
var BaseFlipBookViewer = /*#__PURE__*/function (_BaseViewer) {
  ...
  flipbook_createClass(BaseFlipBookViewer, [{
    ...
    value: function getAnnotationElement(pageNumber) {
      var clean = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
      if (this.app.zoomValue > 1) return this.zoomViewer.getAnnotationElement(pageNumber, clean);else return flipbook_get(flipbook_getPrototypeOf(BaseFlipBookViewer.prototype), "getAnnotationElement", this).call(this, pageNumber, clean);
    }
  }, {
    key: "getTextElement",
    value: function getTextElement(pageNumber) {
    ...
    value: function _getInnerHeight() {
      ...
      var width = this.availablePageWidth(false, true),
        appHeight = this.app.dimensions.maxHeight - this.app.dimensions.padding.height;
      this._containCover = flipbook_utils.contain(viewPort.width * 100, viewPort.height * 100, width, appHeight);
    ...
    value: function availablePageWidth() {
      ...
      pageWidth /= this.pageMode === defaults_DEARVIEWER.FLIPBOOK_PAGE_MODE.SINGLE ? 1 : 2;
    ...
    value: function getTextureSize(param) {
      var viewport = this.getViewPort(param.pageNumber, true);
      var dimen = flipbook_utils.contain(viewport.width * 100, viewport.height * 100, window.devicePixelRatio * this.availablePageWidth(), window.devicePixelRatio * this.availablePageHeight());
      return {
        height: dimen.height,
        width: dimen.width
      };
  ...
  flipbook_createClass(ZoomViewer, [{
    ...
    value: function pagesReady() {
      ...
      if (this.app.options.flipbookFitPages === false) {
        ...
        if (leftViewPort) {
          var leftDimen = flipbook_utils.contain(leftViewPort.width, leftViewPort.height, availableWidth, availableHeight);
        ...
        if (rightViewPort) {
          var rightDimen = flipbook_utils.contain(rightViewPort.width, rightViewPort.height, availableWidth, availableHeight);
...
var BookSheet3D = /*#__PURE__*/function (_BookSheet) {
  ...
  flipbook_3d_createClass(BookSheet3D, [{
    ...
    value: function updateAngle() {
      ...
      this.element.scale.y = this.height / this.element.geometry.parameters.height;
  ...
  flipbook_3d_createClass(FlipBook3D, [{
    ...
    value: function pagesReady() {
      ...
      if (this.app.options.flipbookFitPages === false) {
        ...
        var leftViewPort = this.leftViewport = this.getViewPort(basePage + (this.isBooklet ? 0 : this.isRTL ? 1 : 0)),
          rightViewPort = this.rightViewPort = this.getViewPort(basePage + (this.isBooklet ? 0 : this.isRTL ? 0 : 1));
        if (leftViewPort) {
          var leftDimen = flipbook_3d_utils.contain(leftViewPort.width * 100, leftViewPort.height * 100, this.availablePageWidth(), this.availablePageHeight());
          this.leftSheetWidth = Math.floor(leftDimen.width);
          this.leftSheetHeight = Math.floor(leftDimen.height);
        }
        if (rightViewPort) {
          var rightDimen = flipbook_3d_utils.contain(rightViewPort.width * 100, rightViewPort.height * 100, this.availablePageWidth(), this.availablePageHeight());
          this.rightSheetWidth = Math.floor(rightDimen.width);
          this.rightSheetHeight = Math.floor(rightDimen.height);
        }
        for (var i = 0; i < this.sheets.length; i++) {
    ...
    value: function mouseUp(event) {
      ...
      if (viewer.app.zoomValue === 1) {
        ...
        if (event.ctrlKey === true && isClick) {
          this.zoomOnPoint(point);
        } else if (isClick && point.isInsideSheet === true && viewer.clickAction === flipbook_3d_DV.MOUSE_CLICK_ACTIONS.NAV) {
...
MOCKUP.init = function () {
  ...
  MOCKUP = {
    ...
    clearChild: function clearChild(child) {
      ...
      var texture;
      child.geometry = flipbook_3d_utils.disposeObject(child.geometry);
  ...
  THREE.OrbitControls = function (object, domElement) {
    ...
    this.pan = function (deltaX, deltaY) {
      ...
      if (scope.object instanceof THREE.PerspectiveCamera) {
        ...
        var targetDistance = _offset2.length();
...
var HybridFlipbook3D = /*#__PURE__*/function (_FlipBook3D) {
  ...
  viewer_pros_createClass(HybridFlipbook3D, [{
    ...
    value: function resize() {
      ...
      viewer_pros_get(viewer_pros_getPrototypeOf(HybridFlipbook3D.prototype), "resize", this).call(this);
...
var DV_Share = /*#__PURE__*/function () {
  ...
  controls_createClass(DV_Share, [{
    ...
    value: function init(container, options) {
      ...
      dfShare.urlInput = controls_jQuery('<textarea class="df-share-url">').on("click", function () {
...
defaults_DEARVIEWER.openLightBox = function openLightBox(app) {
  ...
  defaults_DEARVIEWER.activeLightBox.app === undefined || defaults_DEARVIEWER.activeLightBox.app === null || defaults_DEARVIEWER.activeLightBox.app.closeRequested === true || defaults_DEARVIEWER.openLocalFileElement == app // request is through OpenFile function
  ) {
    defaults_DEARVIEWER.activeLightBox.app = controls_utils.disposeObject(defaults_DEARVIEWER.activeLightBox.app);
...
defaults_DEARVIEWER.checkBrowserURLforPDF = function () {
  ...
  if (pdf) {
    pdf = unescape(pdf);
    if (openFlipbook) {
      defaults_DEARVIEWER.openFile(pdf);
    }
  }
  return pdf;
};
...
function createFileInput() {
  var input = defaults_DEARVIEWER.openLocalFileInput = controls_jQuery('<input type="file" accept=".pdf" style="display:none">').appendTo(controls_jQuery("body"));
  input.change(function () {
    ...
    if (files.length) {
      ...
      (_DEARVIEWER$openLocal = defaults_DEARVIEWER['openLocalFileInputChanged']) === null || _DEARVIEWER$openLocal === void 0 || _DEARVIEWER$openLocal.call(defaults_DEARVIEWER, defaults_DEARVIEWER.oldLocalFileObjectURL, file);
      defaults_DEARVIEWER.openFile(defaults_DEARVIEWER.oldLocalFileObjectURL);
    }
  });
}
defaults_DEARVIEWER.openFile = function (src) {
  if (defaults_DEARVIEWER.openLocalFileElement === void 0) {
    defaults_DEARVIEWER.openLocalFileElement = controls_jQuery("<div hidden>").appendTo(controls_jQuery("body")).data('option', defaults_DEARVIEWER.openFileOptions);
  }
  if (src) {
    defaults_DEARVIEWER.openFileOptions.source = src;
    defaults_DEARVIEWER.openFileOptions.pdfParameters = null;
  }
  defaults_DEARVIEWER.openLightBox(defaults_DEARVIEWER.openLocalFileElement);
};
  ...
  function (page) {
    ...
    page.getTextContent().then(function (textData) {
      ...
      textData.items.forEach(function (item) {
        var test = utils.urlify(item.str);
        if (test !== item.str) {
          textItems.push(item);
        }
      });
      ...
      task._textDivs.forEach(function (div) {
        div.innerHTML = utils.urlify(div.innerHTML);
      });
      // console.log(performance.now());
...
DEARFLIP.parseFallBack = function () {
  ...
  dflip_jQuery('._df_button, ._df_thumb, ._df_custom, ._df_book').each(function () {
...
var updateOptions = function updateOptions(options) {
  ...
  if (options.webgl !== void 0) {
    options.is3D = options.webgl = utils.isTrue(options.webgl);
...
dflip_jQuery(document).ready(function () {
  ...
  body.on('click', '.df-element[data-lightbox]', function (event) {
    ...
    event.preventDefault();
    DEARFLIP.openLightBox(dflip_jQuery(this));
  });
...
/* harmony default export */ const dflip = ((/* unused pure expression or super */ null && (DEARFLIP)));
})();
